...
from service.StorageService import upload_file_service, get_file_url_service, get_list_file_url_service
...
class FileController:
    ...
    def upload_product_image(self, id: str, file: UploadFile = File(...), session: Session = Depends(get_db)):
        if get_product_service(session, id) is not None:
            return Response(status_code=200, content=upload_file_service(file.file, f'product/{id}/images/{file.filename}', file.content_type))
        else:
    ...
    def upload_product_image(self, id: str, file: UploadFile = File(...), session: Session = Depends(get_db)):
        if get_product_service(session, id) is not None:
            return Response(status_code=200, content=upload_file_service(file.file, f'product/{id}/{file.filename}', file.content_type))
        else:
            return Response(status_code=404, content="Product not found")