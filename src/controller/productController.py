...
class ProductController:
    ...
    @controller.route.get("/", summary="Get All Products", response_model=list[ProductResponse])
    def get_all_products(self, session: Session = Depends(get_db)) -> list[ProductResponse]:
        return get_products_service(session)

    @controller.route.get("/{id}",summary="Get Specific Product", response_model=ProductResponse)
    ...
    @controller.route.post('/',summary="Creation of a Product",response_model=ProductResponse)
    ...
    @controller.route.delete('/{id}',summary="Delete Data Product", response_model=bool)
